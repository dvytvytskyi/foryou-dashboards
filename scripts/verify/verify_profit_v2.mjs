...
async function verifyProfit() {
    console.log('--- AUDITING FEBRUARY DATA ---');
    ...
    console.log(`Total Feb Expenses FOUND: ${febExp}`);

    // 2. Get Income
    const incRes = await sheets.spreadsheets.values.get({ spreadsheetId: TARGET_ID, range: 'Global_Master_Feed!A1:K3000' });
    const incomes = incRes.data.values || [];
    let febInc = 0;
    incomes.forEach(r => {
        if (r[0] && r[0].includes('2026-02')) febInc += parseFloat((r[9] || '0').replace(/\s/g, '').replace(',', '.'));
    });
    console.log(`Total Feb Income FOUND: ${febInc}`);
    console.log(`EXPLICIT NET: ${febInc - febExp}`);
}
