...
async function updateViewWithDealsLogic() {
    console.log('--- UPDATING VIEW WITH NEW DEALS LOGIC (EOI/SIGN) ---');
    
    // Status IDs to count as "Deals"
    const DEAL_STATUS_IDS = '(70457482, 70457486)';
    ...
    const query = `
        ...
        -- NEW LOGIC FOR DEALS: Count transitions to EOI or Signed statuses
        deal_conversions AS (
          SELECT 
            broker_name,
            DATE_TRUNC(DATE(created_at), MONTH) as month,
            COUNT(DISTINCT lead_id) as actual_deals
          FROM \`crypto-world-epta.foryou_analytics.lead_audit_v3_clean\`
          WHERE status_id IN ${DEAL_STATUS_IDS}
          GROUP BY 1, 2
        ),
        ...
        all_keys AS (
          ...
          UNION DISTINCT
          SELECT broker_name, month FROM deal_conversions
          UNION DISTINCT
        ...
        SELECT 
          ...
          -- DEALS (Conversions to EOI/Sign)
          CAST(IFNULL(d.actual_deals, 0) AS INT64) as fact_deals,
          CAST(IFNULL(t.target_deals, 0) AS INT64) as target_deals,
          SAFE_DIVIDE(CAST(IFNULL(d.actual_deals, 0) AS FLOAT64), CAST(NULLIF(t.target_deals, 0) AS FLOAT64)) as deals_percent,

          CAST(t.crm_quality AS STRING) as crm_quality
          
        FROM all_keys k
        LEFT JOIN lead_facts l ON k.broker_name = l.broker_name AND k.month = l.month
        LEFT JOIN deal_conversions d ON k.broker_name = d.broker_name AND k.month = d.month
    ...
    console.log('SUCCESS: View updated with EOI/Sign-based deal logic.');
}

updateViewWithDealsLogic().catch(console.error);
