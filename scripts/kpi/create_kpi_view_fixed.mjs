...
async function createKpiViewFixed() {
    console.log('--- Creating REPAIRED MASTER KPI View ---');
    ...
    const query = `
        ...
        SELECT 
          k.broker_name as broker,
          k.month,
          
          -- LEADS
          IFNULL(l.actual_leads, 0) as fact_leads,
          IFNULL(t.target_leads, 0) as target_leads,
          ROUND(SAFE_DIVIDE(CAST(IFNULL(l.actual_leads, 0) AS FLOAT64), CAST(NULLIF(t.target_leads, 0) AS FLOAT64)), 4) as leads_percent,

          -- REVENUE
          IFNULL(r.actual_revenue, 0) as fact_revenue,
          IFNULL(t.target_revenue, 0) as target_revenue,
          ROUND(SAFE_DIVIDE(CAST(IFNULL(r.actual_revenue, 0) AS FLOAT64), CAST(NULLIF(t.target_revenue, 0) AS FLOAT64)), 4) as revenue_percent,

          -- DEALS
          IFNULL(r.actual_deals, 0) as fact_deals,
          IFNULL(t.target_deals, 0) as target_deals,
          ROUND(SAFE_DIVIDE(CAST(IFNULL(r.actual_deals, 0) AS FLOAT64), CAST(NULLIF(t.target_deals, 0) AS FLOAT64)), 4) as deals_percent,
    ...
    console.log('SUCCESS: Repaired Master View "FORYOU_KPI_FINAL_MASTER" created.');
}

createKpiViewFixed().catch(console.error);
