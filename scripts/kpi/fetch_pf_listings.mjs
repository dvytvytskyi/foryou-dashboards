...
async function fetchListings() {
    ...
    // Try listing list instead of single ID
    const res = await fetch('https://atlas.propertyfinder.com/v1/listings?perPage=5', {
        headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
    });
    const data = await res.json();
    console.log('Listings Output:', JSON.stringify(data, null, 2));
}
fetchListings();
